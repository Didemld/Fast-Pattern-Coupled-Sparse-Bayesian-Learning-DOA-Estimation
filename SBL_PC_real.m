function [ gamma, report] = SBL_PC_real( A , Y, options )
%
...
for j1 = 1 : options.convergence.maxiter
    ...
    for i_f = 1 : Nfreq
        ...
        Af = squeeze(A(i_f,:,:));
        SigmaYinv = inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)], [1 Nsensor] ) .* Af'));
    ...
    gamma = gamma  .* ((sum( gamma_num   ,1 ) ./...
        sum( abs(gamma_denum_all) ,1 ) ).^(1/options.fixedpoint) ).' ;
