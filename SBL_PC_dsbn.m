function [ x_post, report] = SBL_PC_dsbn( A , Y, options )
%
...
for j1 = 1 : options.convergence.maxiter
    ...
    %% gamma update
    egamma_old = gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)];
    for i_f = 1 : Nfreq
        
        Af = squeeze(A(i_f,:,:));
        SigmaYinv = inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(egamma_old, [1 Nsensor] ) .* Af'));
    ...
    gamma_denum_all = gamma_denum + beta*([gamma_denum(2:end) 0]+ [0 gamma_denum(1:end-1)]...
        - [gamma_num(2:end) 0] - [0 gamma_num(1:end-1)]);
    ...
    egamma = gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)];
...
%     plot((gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]))
%     title('PCSBL')
%     
%     drawnow
...
for i_f = 1 : Nfreq
    ...
    %         x_post(i_f,:,:) = repmat((gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]), [1 Nsnapshot] ) .*...
    %             (Af' / (sigc(i_f) * eye(Nsensor) +...
    %             Af * (repmat((gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]), [1 Nsensor] ) .* Af')) * squeeze2(Y(i_f,:,:)));
...
% 
%  x_post_new = zeros(size(x_post));
%     x_post_new(Ilocs) = x_post(Ilocs);
%     x_post = x_post_new;
%% function return
% Globla minimum
gamma = gamma_min;
