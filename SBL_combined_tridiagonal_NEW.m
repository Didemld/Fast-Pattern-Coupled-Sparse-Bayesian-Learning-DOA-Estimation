function [ x_post, report] = SBL_combined_tridiagonal_NEW( A , Y, options )
%
...
% gamma_b2 = (diag(Af'*squeeze(SCM(1,:,:))*Af,-1));
...
%                 drawnow
      ...
      gamma_a = gamma_a  .* ((sum( gamma_num   ,1 ) ./sum( gamma_denum ,1 ) ).^(1/options.fixedpoint) ).' ;

        
%         for i = 1: Ntheta-1
%             gamma_b1(i)= gamma_b1_Old(i).* ((Af(:,i)'*U*Af(:,i+1))/(Af(:,i)'*V*Af(:,i+1))).^(1/options.fixedpoint);
% %             gamma_b2(i)= gamma_b2_Old(i).* ((Af(:,i+1)'*U*Af(:,i))/(Af(:,i)'*V*Af(:,i))).^(1/options.fixedpoint);
% 
%         end
%          gamma_b2 = (gamma_b1);
...
gamma_all = pinv(Af)*(squeeze(SCM(i_f,:,:))-sigc* eye(Nsensor))*pinv(Af)'; 
gamma_b1 = diag(gamma_all,1);
gamma_b2 = diag(gamma_all,-1);
gamma_a= diag(gamma_all);
% 
for ii = 1:size(gamma_a) 
    if ii ~=Ilocs
        gamma_a(ii) =0;
    end
end
for ii = 1:size(gamma_b1) 
    if ii ~=Ilocs
        gamma_b1(ii) =0;
        gamma_b2(ii) =0;
    end
end
gamma_all = diag(gamma_a)+diag(gamma_b1,1)+diag(gamma_b2,-1);
%% Posterior distribution
% x_post - posterior unbiased mean
for i_f = 1 : Nfreq
    
    x_post(i_f,:,:) = (gamma_all)*...
        (Af' / (sigc(i_f) * eye(Nsensor) + Af * (gamma_all * Afh)) * squeeze2(Y(i_f,:,:)));
...
%     
%     
%     x_post2(i_f,:,:) = repmat(gamma_a, [1 Nsnapshot] ) .*...
%         (Af' / (sigc(i_f) * eye(Nsensor) +...
%         Af * (repmat(gamma_a, [1 Nsensor] ) .* Af')) * squeeze2(Y(i_f,:,:)));
    %     x_post(i_f,:,:) = repmat(gamma_a, [1 Nsnapshot] ) .*...
    %         (Af' / (sigc(i_f) * eye(Nsensor) +...
