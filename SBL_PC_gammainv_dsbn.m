function [ x_post, report] = SBL_PC_gammainv_dsbn( A , Y, options )
%
...
%     gamma= gamma_min;
    ...
    for i_f = 1 : Nfreq
        ...
        Af = squeeze(A(i_f,:,:));
        %         SigmaYinv = inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(1./gamma, [1 Nsensor] ) .* Af'));
        SigmaYinv  = inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]), [1 Nsensor] ) .* Af'));
        ...
        gamma_denum(i_f,:) = ( sum  ( ApSigmaYinv.' .* Af, 1 ) );
    ...
    c1 = ((gammat+beta*[gammat(2:end) 0] +beta* [0 gammat(1:end-1)])./ ([gammat(2:end) 0]+beta*[gammat(3:end) 0 0] +beta* gammat)).^2;
    c2 = ((gammat+beta*[gammat(2:end) 0] +beta* [0 gammat(1:end-1)])./([0 gammat(1:end-1)]+beta*[0 0 gammat(1:end-2)] +beta* gammat)).^2;
...
%     c1=1;
%     c2=1;
    gamma_denum_all = gamma_denum+ beta* c1.*([gamma_denum(2:end) 0] - [gamma_num(2:end) 0])+...
        beta* c2.*([0 gamma_denum(1:end-1)] - [0 gamma_num(1:end-1)]);
    
    egamma_old = 1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]);

%     % Fixed point Eq. update
%     gamma = gamma  .* ((sum( abs(gamma_denum_all)   ,1 ) ./...eee
%         sum( gamma_num ,1 ) ).^(1/options.fixedpoint) ).' ;
% %     gamma(gamma == Inf) = realmax;

  K = gamma_num - gamma_denum_all;
    % Fixed point Eq. update
    gamma = abs(a./(b-K)).';
...
%         egamma = 1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]);
        egamma = 1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]);

        % locate same peaks for all frequencies
        [ ~ , Ilocs] = findpeaks(egamma,'SORTSTR','descend','NPEAKS',Nsource);
        Apeak      = A(:,:,Ilocs);
...
%     plot(1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]))
%     title('PCSBL gammainv')
%     drawnow
%     
...
%     egamma_old = 1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]);
    ...
    errornorm(j1) = norm (egamma - egamma_old, 1 ) / norm ( egamma, 1 );
...
%         x_post(i_f,:,:) = repmat(1./gamma, [1 Nsnapshot] ) .*...
%             (Af' / (sigc(i_f) * eye(Nsensor) +...
%             Af * (repmat(1./gamma, [1 Nsensor] ) .* Af')) * squeeze2(Y(i_f,:,:)));
%     
    x_post(i_f,:,:) = repmat(1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]), [1 Nsnapshot] ) .*...
        (Af' / (sigc(i_f) * eye(Nsensor) +...
        Af * (repmat(1./(gamma+beta*[gamma(2:end); 0] +beta* [0; gamma(1:end-1)]), [1 Nsensor] ) .* Af')) * squeeze2(Y(i_f,:,:)));
