function [ x_post, report] = SBL_combined_tridiagonal_PC( A , Y, options )
%
...
for j1 = 1 : options.convergence.maxiter
    ...
    for i_f = 1 : Nfreq
        ...
        egamma_old = (gamma_a+beta*[gamma_a(2:end); 0] +beta* [0; gamma_a(1:end-1)]);
        
        SigmaYinv  = inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(gamma_a+beta*[gamma_a(2:end); 0] +beta* [0; gamma_a(1:end-1)], [1 Nsensor] ) .* Afh +...
            repmat([gamma_b1;0], [1 Nsensor]) .* [Afh(2:end,:); zeros(1,Nsensor)]+...
            repmat([0;gamma_b2], [1 Nsensor]) .* [zeros(1,Nsensor);Afh(1:end-1,:)]));
        ...
        gamma_denum_all = gamma_denum + beta*([gamma_denum(2:end) 0]+ [0 gamma_denum(1:end-1)]- [gamma_num(2:end) 0] - [0 gamma_num(1:end-1)]);
        ...
        gamma_a = gamma_a  .* ((sum( gamma_num   ,1 ) ./sum( abs(gamma_denum_all) ,1 ) ).^(1/options.fixedpoint) ).' ;
              for i = 1: Ntheta-1
            gamma_b1(i)= gamma_b1_Old(i)* ((Af(:,i)'*U*Af(:,i+1))/(Af(:,i)'*V*Af(:,i))).^(1/options.fixedpoint);
        end
        gamma_b2 = gamma_b1;
    ...
    egamma = (gamma_a+beta*[gamma_a(2:end); 0] +beta* [0; gamma_a(1:end-1)]);
            egamma_old = (gamma_a+beta*[gamma_a(2:end); 0] +beta* [0; gamma_a(1:end-1)]);
...
for i_f = 1 : Nfreq
    ...
    x_post(i_f,:,:) = (repmat(gamma_a, [1 Nsnapshot] ) +repmat([gamma_b1;0], [1 Nsnapshot]) +repmat([0;gamma_b2], [1 Nsnapshot])) .*...
        (Af' / (sigc(i_f) * eye(Nsensor) +...
        Af * (repmat(gamma_a, [1 Nsensor] ) .* Afh +repmat([gamma_b1;0], [1 Nsensor]) .* [Afh(2:end,:); zeros(1,Nsensor)]+repmat([0;gamma_b2], [1 Nsensor]) .* [zeros(1,Nsensor);Afh(1:end-1,:)])) * squeeze2(Y(i_f,:,:)));
